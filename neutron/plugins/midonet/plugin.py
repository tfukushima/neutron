...
class MidonetPluginV2(db_base_plugin_v2.NeutronDbPluginV2,
    ...
    def _initialize_port_chains(self, context, port,
        ...
        """ Initializes ingress and egress security rule chains for
        the specified port.
        ...
        # reverse order.
        #
        # WARNING: When making changes to the structure of the rule
        # chains, it may be necessary to modify the pos values in the
        # function _add_sg_jumps_to_port_chains to make sure that the
        # security group jump rules are inserted in the correct
        # positions when modifying a port's SG assignments.
        ...
        for fixed_ip in port["fixed_ips"]:
            subnet = self._get_subnet(context, fixed_ip["subnet_id"])
            ipv6 = (subnet["ip_version"] == 6)
            dl_type, mask = (0x86dd, "/128") if ipv6 else (0x0800, "/32")
            self._add_chain_rule(in_chain, action="drop",
                                 src_addr=fixed_ip["ip_address"] + mask,
                                 inv_nw_src=True, dl_type=dl_type,
                                 position=1)
    ...
    def _add_sg_jumps_to_port_chains(self, port_inbound, port_outbound,
        ...
        """ Add rules to jump from port_inbound to sg_egress and from
        port_outbound to sg_ingress at the appropriate place in each
        port chain. Port_inbound and sg_egress are the port and
        security group chains for traffic out of a VM and into the
        network, while port_outbound and sg_ingress are for traffic out
        of the network and into a VM. """

        # Insert jumps into 4th position for inbound chain and 2nd
        # position for outbound chain. Each chain has a rule to match
        # reverse flow which must precede the SG jump rules, and the
        # inbound chain also has two anti-spoofing rules.
        for port_chain, sg_chain, pos in [(port_inbound, sg_egress, 4),
                                          (port_outbound, sg_ingress, 2)]:
