...
class DynamicRoutingAgentSchedulerDbMixin(
    dr_as.DynamicRoutingSchedulerPluginBase,
    ...
    def dr_rpc_notifier(self):
        if not hasattr(self, '_dr_rpc_notifier'):
            self._dr_rpc_notifier = dr_rpc.DynamicRoutingAgentNotifyAPI()
    ...
    def add_routingpeer_to_dr_agent(self, context, agent_id, routingpeer_id):
        ...
        self.dr_rpc_notifier.add_routingpeer(context,
                                             routingpeer_dict,
                                             agent_db.host)
    ...
    def list_agents_on_routinginstance(self, context, routinginstance_id):
        ...
        query = query.filter(RoutingInstanceAgentBinding.routinginstance_id ==
                routinginstance_id)
    ...
    def add_agent_to_routinginstance(self, context, routinginstance_id,
        ...
        with context.session.begin(subtransactions=True):
            ...
            try:
                binding = query.one()
                raise dr_as.AgentAssociatedRoutingInstance(
                    agent_id=binding.agent_id)
            except exc.NoResultFound:
                pass
    ...
    def _get_dr_bindings_hosting_peers(self, context, routingpeer_id):
        query = context.session.query(RoutingPeerAgentBinding)
